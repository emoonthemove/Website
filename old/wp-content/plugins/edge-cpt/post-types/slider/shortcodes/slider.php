...
class Slider implements Lib\ShortcodeInterface {
    ...
    public function render($atts, $content = null) {
        ...
        if ($slider != "") {
            ...
            if($illustrator_edge_options['smooth_page_transitions'] == "yes" && $illustrator_edge_options['smooth_pt_spinner_type'] != "") {
                $ajax_loader = '<div class="edgtf-st-loader"><div class="edgtf-st-loader1">' . illustrator_edge_loading_spinners(true) . '</div></div>';
            }else{
                $ajax_loader = '<div class="edgtf-st-loader"><div class="edgtf-st-loader1">'. illustrator_edge_loading_spinner_pulse() .'</div></div>';
            }

            /* set slider preloader - end */

            /* set padding for slider arrows - start */
            $slider_arrows_padding = "padding-top: " . esc_attr($this->illustrator_edge_get_slider_navigation_padding()) . "px;";
            /* set padding for slider arrows - end */

            $html .= '<div id="edgtf-' . esc_attr($slider) . '" ' . $slider_anchor . ' class="carousel slide ' . esc_attr($animation_type_class . ' ' . $full_screen_class . ' ' . $responsive_height_class . ' ' . $height_class . ' ' . $auto_start_class . ' ' . $header_effect_class . ' ' . $slider_numbers_class .  ' ' . $slider_thumbs_class) . ' " ' . $slide_animation_timeout . ' ' . $data_height . ' ' . $data_parallax_effect . ' '.illustrator_edge_get_inline_style($slide_holder_height).'><div class="edgtf-slider-preloader">'.$ajax_loader.'</div>';
            $html .= '<div class="carousel-inner ' . esc_attr($slider_css_position_class) . '" '.illustrator_edge_get_inline_style($carouselinner_height).' '.$data_parallax_transform.'>';
            ...
            if (have_posts()) : $postCount = 0;
                while (have_posts()) : the_post();
					...
					$slide_image_tablet_responsive = esc_url(get_post_meta(get_the_ID(), "edgtf_slide_image_tablet", true));
					$slide_image_phone_responsive = esc_url(get_post_meta(get_the_ID(), "edgtf_slide_image_phone", true));
					$image_holder_data = array();
					if($image != '') {
						$image_holder_data[] = 'data-original-image = '.$image;
					}
					if($slide_image_tablet_responsive != '') {
						$image_holder_data[] = 'data-responsive-tablet-image = '.$slide_image_tablet_responsive;
					}
					if($slide_image_phone_responsive != '') {
						$image_holder_data[] = 'data-responsive-phone-image = '.$slide_image_phone_responsive;
					}
                    ...
                    if ($slide_type == 'video') {
                        ...
                        $html .= '<div class="edgtf-video"><div class="edgtf-mobile-video-image" '.illustrator_edge_get_inline_style('background-image: url(' . esc_url($video_image) . ')').'></div><div class="edgtf-video-overlay';
						...
						$html .= '<video id="edgtf-slider-video-'. get_the_ID() .'" class="edgtf-video-element" data-slide-id='.get_the_ID().' width="1920" height="800" poster="' . esc_url($video_image) . '" controls="controls" preload="auto" loop muted>';
						if (!empty($video_webm)) {
                        ...
                        $html .= '<div class="edgtf-image" '.illustrator_edge_get_inline_style('background-image:url(' . esc_url($image) . ')').'>';
                        if ($slider_thumbs == 'no') {
                            $html .= '<img src="' . esc_url($image) . '" alt="' . esc_attr($title) . '">';
                        }

                        if ($image_overlay_pattern !== "") {
                            $html .= '<div class="edgtf-image-pattern" '.illustrator_edge_get_inline_style('background: url(' . esc_url($image_overlay_pattern) . ') repeat 0 0').'></div>';
                        }
                        $html .= '</div>';
                    }
                    ...
                    if ($element_positions == 'custom') {
                        $html .= 
                            '<div class="edgtf-slider-elements-container edgtf-custom-elements '.( $frame_in_grid == 'yes' ? 'edgtf-grid' : '' ).'" '.illustrator_edge_get_inline_style( ($frame_in_grid == 'no' && (($meta_temp = get_post_meta(get_the_ID(), "edgtf_slide_holder_frame_width", true)) != '') ? 'width: '.$meta_temp.'%; ' : '') . 'padding-top: ' . esc_attr($this->illustrator_edge_get_slider_navigation_padding()) . 'px;').' '.$slide_elements_data_start.' '.$slide_elements_data_end.($frame_in_grid == 'yes' ? '' : ' data-width-mobile="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_mobile', true)).'" data-width-tablet-p="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_tablet_p', true)).'" data-width-tablet-l="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_tablet_l', true)).'" data-width-laptop="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_laptop', true)).'" data-width-desktop="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_desktop', true)).'"').'>'.
                                '<div class="edgtf-slider-elements-container-inner" ' . illustrator_edge_get_inline_style((($meta_temp = get_post_meta(get_the_ID(), "edgtf_slide_holder_frame_height", true)) != '') ? 'padding-bottom: '.$meta_temp.'%;' : '') . '>' .
                                    '<div class="edgtf-slider-elements-holder-frame" ' . illustrator_edge_get_inline_style('width: 100%; height: 100%').' data-default-width="'.esc_attr($def_width).'">'
                        ;
                    }
                    else {
                        $html .= 
                            '<div class="edgtf-slider-elements-container" '.illustrator_edge_get_inline_style('width: 100%; box-sizing: border-box; height: 100%; ' . 'padding-top: ' . esc_attr($this->illustrator_edge_get_slider_navigation_padding()) . 'px;').' '.$slide_elements_data_start.' '.$slide_elements_data_end.($frame_in_grid == 'yes' ? '' : ' data-width-mobile="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_mobile', true)).'" data-width-tablet-p="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_tablet_p', true)).'" data-width-tablet-l="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_tablet_l', true)).'" data-width-laptop="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_laptop', true)).'" data-width-desktop="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_desktop', true)).'"').'>'.
                                '<div class="edgtf-slider-elements-container-inner" '.illustrator_edge_get_inline_style('height: 100%;').'>' .
                                    '<div class="edgtf-slider-elements-holder-frame '.( $frame_in_grid == 'yes' ? 'edgtf-grid' : '' ).'" '.illustrator_edge_get_inline_style( ($frame_in_grid == 'no' && (($meta_temp = get_post_meta(get_the_ID(), "edgtf_slide_holder_frame_width", true)) != '') ? 'width: '.$meta_temp.'%; ' : '').'top: 50%; left: 50%; -webkit-transform: translate3d(-50%,-50%,0); transform: translate3d(-50%,-50%,0);').' data-default-width="'.esc_attr($def_width).'">'
                        ;
                    }
                                ...
                                case 'text':
                                    ...
                                    if ($text_options == 'advanced') {
                                        ...
                                        if ($slide_element['slideelementtextbgndcolor'] != '') {
                                            ...
                                            $style_string .= 'background-color: '.illustrator_edge_rgba_color($slide_element_bg_color, $slide_element_bg_transparency).';';
                                        }
                                    ...
                                    $slide_element_html .= 
                                        '<div class="'.$class_string.' '.($element_positions.'-image').'" '.$data_string.' style="'.$style_string.'"><div class="edgtf-slide-element-inner" style="'.$inner_style_string.'">'.
                                            ($slide_element['slideelementimagelink'] != '' ? '<a class="edgtf-slide-element-wrapper-link inheriting" href="'.esc_url($slide_element['slideelementimagelink']).'" target="'.esc_attr($slide_element['slideelementimagetarget']).'">' : '') .
                                                '<img '. (($element_positions != 'custom' && $slide_element['slideelementimagewidth'] != '') ? 'style="width: '.esc_attr($slide_element['slideelementimagewidth']).'%" ' : '') .'src="'.(isset($slide_element['slideelementimageurl']) ? $slide_element['slideelementimageurl'] : '').'" alt="">'. 
                                ...
                                case 'button':
                                    ...
                                    if ($slide_element['slideelementbuttoniconpack'] != '') {
                                        ...
                                        if ($button_params['icon_pack'] != 'no_icon') {
                                            $iconPackName = illustrator_edge_icon_collections()->getIconCollectionParamNameByKey($button_params['icon_pack']);
                                    ...
                                    $slide_element_html .= '<div class="'.$class_string.'" '.$data_string.' style="'.$style_string.'"><div class="edgtf-slide-element-inner" style="'.$inner_style_string.'">'.illustrator_edge_get_button_html($button_params).'</div></div>';
                                break;
                ...
                elseif(isset($slider_meta['slider_nav_thumbs']) && $slider_meta['slider_nav_thumbs'] == 'yes'){
                    ...
                    if (have_posts()) : $postCount = 0;
                        while (have_posts()) : the_post();
                            ...
                            $slide_type = get_post_meta(get_the_ID(), "edgtf_slide_background_type", true);
                            if( $slide_type == 'image'){
                                $img .= illustrator_edge_generate_thumbnail('',esc_url(get_post_meta(get_the_ID(), "edgtf_slide_image", true)),'160','100');
                            }elseif($slide_type == 'video'){                                
                                $img .= illustrator_edge_generate_thumbnail('',esc_url(get_post_meta(get_the_ID(), "edgtf_slide_video_image", true)),'160','100');
                            }                           
                            $html .= '>';
                            $html .= $img;
                            $html .= '</li>';
     ...
     **/
    function illustrator_edge_get_slider_item_padding() {
        return apply_filters('illustrator_edge_slider_item_padding', 0);
    }

    /**
     * Function that returns slider navigation pading
     **/
    function illustrator_edge_get_slider_navigation_padding() {
        return apply_filters('illustrator_edge_slider_navigation_padding', 0);
    }
}