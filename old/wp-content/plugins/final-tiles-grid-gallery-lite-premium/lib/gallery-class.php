    ...
    //          print_r($posts);
		...
		public function getPosts()
		{
			...
			if(ftg_fs()->is_plan_or_trial__premium_only("ultimate"))
			{
				...
				$query  = "SELECT distinct p.* FROM $wpdb->posts p\n";
				$query .= "INNER JOIN $wpdb->postmeta pm ON pm.post_id = p.ID AND pm.meta_key = '_thumbnail_id'\n";
				$query .= "LEFT JOIN $wpdb->term_relationships tr ON tr.object_id = p.ID\n";
				...
				{
					$join = $relation == "OR" ? "LEFT" : "INNER";
					$idx = 1;
					foreach(explode(",", $this->gallery->post_taxonomies) as $item)
					{
						$i = $idx++;
						$t = explode("|", $item);
						$query .= "$join JOIN $wpdb->term_taxonomy tm_{$i} ON tm_{$i}.term_taxonomy_id=tr.term_taxonomy_id AND tm_{$i}.taxonomy = '". $t[0] ."' AND tr.term_taxonomy_id=" . $t[1] . " \n";
					}					
				}
				...
				if($this->gallery->post_types) {
					...
					$query .= "AND p.post_type IN (".implode(",", $fmt_types) . ")";
				} else {
				...
				if(isset($this->gallery->post_categories) && !empty($this->gallery->post_categories)) {
					...
					$query .= "AND tr.term_taxonomy_id IN (". implode(",", $cat_id) .") ";
				}
