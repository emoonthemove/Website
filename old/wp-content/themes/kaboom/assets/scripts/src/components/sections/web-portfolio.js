...
const design = '.design-gallery'

function go(cat) {

    let gallery = document.querySelector(cat)
    
    if (gallery) {

        let link = gallery.dataset.link
        let more = gallery.parentElement.parentElement.querySelector('.more-projects')
        ...
        function displayProject() {
            ...
            for (var i = projectCurrent; i < projectCurrent + projectLimit; i++){
                ...
                if (contentSplit[i]) {
                    gallery.insertAdjacentHTML('beforeend', contentSplit[i]);
                }
        ...
        // Fetch category
        function initProjects(link) {

            // const url = `http://kaboom.test/wp-json/wp/v2/pages/${link}`;
            const url = `https://kaboom.bg/wp-json/wp/v2/pages/${link}`;
            fetch(url)
            .then((resp) => resp.json())
            .then(function(data) {
...
open.forEach(item => {
    item.addEventListener('click', event => {
        if (item.querySelector('.web__arrow--active')) {
            let category = `.${item.nextElementSibling.dataset.slug}-gallery`
            go(category)
        }
    })
});
