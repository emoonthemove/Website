...
if(!function_exists('illustrator_edge_get_title')) {
    /**
     * Loads title area HTML
     */
    function illustrator_edge_get_title() {
        $id = illustrator_edge_get_page_id();

        extract(illustrator_edge_title_area_height());
        extract(illustrator_edge_title_area_background());
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_show_title_area_meta", true)) !== ""){
            ...
            $show_title_area = illustrator_edge_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_type_meta", true)) !== ""){
            ...
            $type = illustrator_edge_options()->getOptionValue('title_area_type');
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_enable_breadcrumbs_meta", true)) !== ""){
            ...
            $enable_breadcrumbs = illustrator_edge_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
        }
        ...
        $parameters = apply_filters('illustrator_edge_title_area_height_params', $parameters);

        illustrator_edge_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('illustrator_edge_get_title_text')) {
    /**
     * Function that returns current page title text. Defines illustrator_edge_title_text filter
     ...
     */
    function illustrator_edge_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'illustrator');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'illustrator') . " " . get_the_author();
        }
		...
		elseif (is_search()) {
			...
			$title = esc_html__('Search Results for', 'illustrator').': '.get_search_query();
		}

        //is current page 404?
        elseif (is_404()) {
            //is 404 title text set in theme options?
            if(illustrator_edge_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = illustrator_edge_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'illustrator');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(illustrator_edge_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
        ...
        elseif(illustrator_edge_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','illustrator');
        }
        ...
        $title = apply_filters('illustrator_edge_title_text', $title);

        return $title;
    }
}

if(!function_exists('illustrator_edge_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see illustrator_edge_get_title_text()
     */
    function illustrator_edge_title_text() {
        echo illustrator_edge_get_title_text();
    }
}

if(!function_exists('illustrator_edge_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     *
     */
    function illustrator_edge_subtitle_text() {
        $id = illustrator_edge_get_page_id();
        $subtitle 	= '';
...
if(!function_exists('illustrator_edge_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see illustrator_edge_is_product_category()
     * @see get_search_query()
     */
    function illustrator_edge_custom_breadcrumbs() {
        global $post;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = illustrator_edge_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="edgtf-delimiter" '.illustrator_edge_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
        $home = get_bloginfo('name'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="edgtf-current">'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.illustrator_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a '.illustrator_edge_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.illustrator_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.illustrator_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || illustrator_edge_is_product_category()) {
                ...
                $output .= $before . esc_html__('Search results for ', 'illustrator') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a '.illustrator_edge_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a '.illustrator_edge_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
                ...
                $output .= '<a '.illustrator_edge_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a '.illustrator_edge_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
                ...
                $output .= $before . esc_html__('Posts tagged ','illustrator') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                global $authordata;
                $output .= $before . esc_html__('Articles posted by ','illustrator') . $authordata->display_name . $after;

            } elseif ( is_404() ) {
                $output .= $before . esc_html__('Error 404','illustrator') . $after;
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'illustrator') . ' ' . get_query_var('paged') . ")" . $after;
...
if(!function_exists('illustrator_edge_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function illustrator_edge_get_title_area_height() {
        ...
        if(($meta_temp = get_post_meta($id, 'edgtf_title_area_type_meta', true)) != '') {
            ...
            $title_type = illustrator_edge_options()->getOptionValue('title_area_type');
        }

        $title_height = 180; // default title height without header height
        ...
        } elseif(illustrator_edge_options()->getOptionValue('title_area_height') !== '') {
            $title_height = illustrator_edge_options()->getOptionValue('title_area_height');
        }

        return apply_filters('illustrator_edge_title_area_height', $title_height);
    }
}

if(!function_exists('illustrator_edge_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function illustrator_edge_get_title_content_padding() {
        return apply_filters('illustrator_edge_title_content_padding', 0);
    }
}


if(!function_exists('illustrator_edge_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function illustrator_edge_title_area_height() {
        $id = illustrator_edge_get_page_id();
        $title_height_and_padding = array();
        $title_height          = illustrator_edge_get_title_area_height();
        $header_height_padding = illustrator_edge_get_title_content_padding();
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_image_responsive_meta", true)) != "") {
            ...
            $is_img_responsive = illustrator_edge_options()->getOptionValue('title_area_background_image_responsive');
        }

        if(($meta_temp = get_post_meta($id, "edgtf_title_area_vertial_alignment_meta", true)) !== '') {
            $title_vertical_alignment = $meta_temp;
        }else{
            $title_vertical_alignment = illustrator_edge_options()->getOptionValue('title_area_vertial_alignment');;
        }
...
if(!function_exists('illustrator_edge_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/

    function illustrator_edge_title_area_background() {
        $id = illustrator_edge_get_page_id();
        $show_title_img = true;
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_color_meta", true)) != ""){
            ...
            $background_color = illustrator_edge_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_image_meta", true)) != ""){
            ...
            $background_image = illustrator_edge_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = illustrator_edge_get_image_dimensions($background_image);
