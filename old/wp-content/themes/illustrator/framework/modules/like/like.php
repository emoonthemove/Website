<?php
class IllustratorEdgeLike {

	private static $instance;

	private function __construct() {
		add_action('wp_enqueue_scripts', array( $this, 'enqueue_scripts'));
		add_action('wp_ajax_illustrator_edge_like', array( $this, 'ajax'));
		add_action('wp_ajax_nopriv_illustrator_edge_like', array( $this, 'ajax'));
	}
		...
		switch($action) {
			...
			case 'get':
				...
				if (is_singular('post')){
					switch ($like_count) {
						case 0:
							$like_text = esc_html__(' Likes','illustrator');
							break;
						case 1:
							$like_text = esc_html__(' Like','illustrator');
							break;
						default:
							$like_text = esc_html__(' Likes','illustrator');
							break;
	...
	public function add_like() {
		...
		$title = esc_html__('Like this', 'illustrator');
		if( isset($_COOKIE['edgtf-like_'. $post->ID]) ){
			$class = 'edgtf-like liked';
			$title = esc_html__('You already like this!', 'illustrator');
		}
...
if ( !function_exists( 'illustrator_edge_create_like' ) ) {

	function illustrator_edge_create_like() {
		IllustratorEdgeLike::get_instance();
	}

	add_action('after_setup_theme', 'illustrator_edge_create_like');
}
