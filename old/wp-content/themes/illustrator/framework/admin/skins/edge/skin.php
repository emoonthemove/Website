...
class EdgeSkin extends IllustratorEdgeSkinAbstract {
    /**
     * Skin constructor. Hooks to illustrator_edge_admin_scripts_init and illustrator_edge_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'edge';

        //hook to
        add_action('illustrator_edge_admin_scripts_init', array($this, 'registerStyles'));
        add_action('illustrator_edge_admin_scripts_init', array($this, 'registerScripts'));

        add_action('illustrator_edge_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('illustrator_edge_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('illustrator_edge_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('illustrator_edge_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            illustrator_edge_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            illustrator_edge_register_skin_style($styleHandle, $stylePath);
        }
    ...
    public function renderOptions() {
        ...
        $tab    = illustrator_edge_get_admin_tab();
        $active_page = $illustrator_edge_Framework->edgtOptions->getAdminPageFromSlug($tab);
     ...
     *
*@param IllustratorEdgeAdminPage $page current page to load
     *
     * @see EdgeSkinAbstract::loadTemplatePart()
     */
