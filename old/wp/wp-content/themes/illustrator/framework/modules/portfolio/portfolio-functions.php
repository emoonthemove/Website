...
if(!function_exists('illustrator_edge_single_portfolio')) {
    function illustrator_edge_single_portfolio() {
        $portfolio_template = illustrator_edge_get_portfolio_single_type();
        $holder_class = $portfolio_template;
        $fullwidth = false;
        $related_slug = '';

        if (illustrator_edge_string_ends_with($portfolio_template,'-right')){
        	$holder_class = illustrator_edge_filter_suffix($portfolio_template,'-right');
        }
        ...
        if ($portfolio_template == 'gallery') {
            $params['holder_class'][] = 'edgtf-portfolio-gallery-' . illustrator_edge_options()->getOptionValue('portfolio_single_numb_columns');
        }

        illustrator_edge_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
    }
}

if(!function_exists('illustrator_edge_get_portfolio_single_type')) {
    function illustrator_edge_get_portfolio_single_type() {
        return illustrator_edge_get_meta_field_intersect('portfolio_single_template');
    }
}

if(!function_exists('illustrator_edge_get_portfolio_single_media')) {
    function illustrator_edge_get_portfolio_single_media() {
	    ...
	    $single_type = illustrator_edge_get_portfolio_single_type();
        if($image_ids !== '') {
		        ...
		        foreach($image_ids as $image_id) {
			        ...
			        switch($size){
				        case 'edgtf-large-height-masonry-item' :
					        $img_size = 'illustrator_edge_large_height';
					        break;
				        case 'edgtf-large-width-masonry-item' :
					        $img_size = 'illustrator_edge_large_width';
					        break;
				        case 'edgtf-large-width-height-masonry-item' :
					        $img_size = 'illustrator_edge_large_width_height';
					        break;
				        default:
					        $img_size = 'illustrator_edge_square';
					        break;
			        }
        ...
        if(is_array($videos) && count($videos)) {
            usort($videos, 'illustrator_edge_compare_portfolio_videos');
            foreach($videos as $video) {
                ...
                if(!empty($video['portfoliovideotype'])) {
                    ...
                    $media['video_url']   = illustrator_edge_portfolio_get_video_url($video);
...
if(!function_exists('illustrator_edge_portfolio_get_video_url')) {
    function illustrator_edge_portfolio_get_video_url($video) {
...
if(!function_exists('illustrator_edge_portfolio_get_media_html')) {
    function illustrator_edge_portfolio_get_media_html($media) {
        global $wp_filesystem;
        $params = array();

        //For adding image overlay in gallery template type
        $params['gallery'] = (illustrator_edge_get_portfolio_single_type() == 'gallery') ? true : false;
        $params['full_screen_slider'] = (illustrator_edge_get_portfolio_single_type() == 'full-screen-slider') ? true : false;

		if($media['type'] == 'image') {
			if(!$params['full_screen_slider']) {
				$params['lightbox'] = illustrator_edge_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
			}
        ...
        if(in_array($media['type'], array('youtube', 'vimeo'))) {
			if(!$params['full_screen_slider']) {
				$params['lightbox'] = illustrator_edge_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
			}
        ...
        illustrator_edge_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
    }
}

if(!function_exists('illustrator_edge_compare_portfolio_videos')) {
    /**
     ...
     */
    function illustrator_edge_compare_portfolio_videos($a, $b) {
...
if(!function_exists('illustrator_edge_compare_portfolio_options')) {
    /**
     ...
     * @return int result of comparison
     */
    function illustrator_edge_compare_portfolio_options($a, $b) {
...
if(!function_exists('illustrator_edge_portfolio_get_info_part')) {
    function illustrator_edge_portfolio_get_info_part($part) {
        $portfolio_template = illustrator_edge_get_portfolio_single_type();

        illustrator_edge_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template, array('type'=>$portfolio_template));
    }
}