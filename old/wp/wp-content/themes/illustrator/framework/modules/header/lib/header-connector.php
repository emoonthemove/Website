<?php
namespace IllustratorEdge\Modules\Header\Lib;
/**
...
class HeaderConnector {
    ...
    public function connect($config = array()) {
        do_action('illustrator_edge_pre_header_connect');
        ...
        if(!empty($config['affect_content'])) {
            add_filter('illustrator_edge_content_elem_style_attr', array($this, 'contentMarginFilter'));
        }

        if(!empty($config['affect_title'])) {
            add_filter('illustrator_edge_title_content_padding', array($this, 'titlePaddingFilter'));
        }

        if(!empty($config['affect_slider'])) {

            add_filter('illustrator_edge_slider_item_padding', array($this, 'sliderItemPaddingFilter'));
            add_filter('illustrator_edge_slider_navigation_padding', array($this, 'sliderNavigationPaddingFilter'));
        }

        do_action('illustrator_edge_after_header_connect');
    }
     ...
     *
     * Hooks to illustrator_edge_title_content_padding filter
     *
     * @return int
     */
     ...
     *
     * Hooks to illustrator_edge_slider_item_padding filter
     *
     * @return int
     */
