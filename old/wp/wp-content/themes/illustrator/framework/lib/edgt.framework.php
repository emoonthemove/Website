<?php
/*
   Class: IllustratorEdgeFramework
...
class IllustratorEdgeFramework {
    ...
    private function __construct() {
        $this->edgtOptions = IllustratorEdgeOptions::get_instance();
        $this->edgtMetaBoxes = IllustratorEdgeMetaBoxes::get_instance();
    }
	...
	public function setSkin(IllustratorEdgeSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class IllustratorEdgeSkinManager
 *
...
/**
 * Class IllustratorEdgeSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class IllustratorEdgeSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(illustrator_edge_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function getMenuIcon($icon) {
		...
		return illustrator_edge_get_skin_uri() . '/assets/img/admin-logo-icon.png';
	}
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.edgtSCIcon = '<?php echo illustrator_edge_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class IllustratorEdgeOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $illustrator_edge_options)) {
            return $illustrator_edge_options[$key];
...
/*
   Class: IllustratorEdgeAdminPage
   A class that initializes Edge Admin Page
*/
class IllustratorEdgeAdminPage implements iIllustratorEdgeLayoutNode {
    ...
    function __construct($slug="", $title="", $icon = "") {
        $this->layout = array();
        $this->factory = new IllustratorEdgeFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iIllustratorEdgeRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: IllustratorEdgeMetaBoxes
...
/*
   Class: IllustratorEdgeMetaBox
   A class that initializes Edge Meta Box
*/
class IllustratorEdgeMetaBox implements iIllustratorEdgeLayoutNode {
	...
	public function setTitle($label) {
		...
		return $illustrator_edge_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if ( ! function_exists( 'illustrator_edge_init_framework_variable' ) ) {

	function illustrator_edge_init_framework_variable() {

		global $illustrator_edge_Framework;

		$illustrator_edge_Framework = IllustratorEdgeFramework::get_instance();
		$edgtSkinManager = new IllustratorEdgeSkinManager();
		$illustrator_edge_Framework->setSkin($edgtSkinManager->getSkin());
	}

	add_action('illustrator_edge_before_options_map', 'illustrator_edge_init_framework_variable' );
}
?>