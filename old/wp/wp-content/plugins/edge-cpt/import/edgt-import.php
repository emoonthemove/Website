...
class IllustratorEdgeImport {
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'edgt_options_illustrator', $options);
    ...
    function edgt_admin_import() {
        if (edge_cpt_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                array($illustrator_edge_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'illustrator_edge_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'illustrator_edge_enqueue_admin_styles');
...
function edgt_init_import_object(){
    global $illustrator_edge_import_object;
    $illustrator_edge_import_object = new IllustratorEdgeImport();
}

add_action('init', 'edgt_init_import_object');

if(!function_exists('illustrator_edge_dataImport')){
    function illustrator_edge_dataImport(){
        global $illustrator_edge_import_object;

        if ($_POST['import_attachments'] == 1)
            $illustrator_edge_import_object->attachments = true;
        else
            $illustrator_edge_import_object->attachments = false;

        $folder = "illustrator/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $illustrator_edge_import_object->import_content($folder.$_POST['xml']);

        die();
    }

    add_action('wp_ajax_edgt_dataImport', 'illustrator_edge_dataImport');
}

if(!function_exists('illustrator_edge_widgetsImport')){
    function illustrator_edge_widgetsImport(){
        global $illustrator_edge_import_object;
        ...
        $illustrator_edge_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_edgt_widgetsImport', 'illustrator_edge_widgetsImport');
}

if(!function_exists('illustrator_edge_optionsImport')){
    function illustrator_edge_optionsImport(){
        global $illustrator_edge_import_object;
        ...
        $illustrator_edge_import_object->import_options($folder.'options.txt');

        die();
    }

    add_action('wp_ajax_edgt_optionsImport', 'illustrator_edge_optionsImport');
}

if(!function_exists('illustrator_edge_otherImport')){
    function illustrator_edge_otherImport(){
        global $illustrator_edge_import_object;
        ...
        $illustrator_edge_import_object->import_options($folder.'options.txt');
        $illustrator_edge_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $illustrator_edge_import_object->import_menus($folder.'menus.txt');
        $illustrator_edge_import_object->import_settings_pages($folder.'settingpages.txt');

        die();
    }

    add_action('wp_ajax_edgt_otherImport', 'illustrator_edge_otherImport');
}